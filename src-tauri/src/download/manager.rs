...
impl DownloadManager {
    ...
    pub async fn start_task(&self, task_id: &str) -> Result<(), DownloadError> {
        let task = {
            ...
            tasks.get(task_id).cloned().ok_or_else(|| DownloadError::TaskNotFound(task_id.to_string()))?
        };
        
        // 检查当前活跃任务数量
        {
            let active_tasks = self.active_tasks.lock().map_err(|_| DownloadError::LockError)?;
        ...
        // 发送开始事件
        let _ = self.event_sender.send(DownloadEvent::Started(task_id.to_string())).await;
        ...
        for i in 0..task.segments {
            ...
            let handle = spawn(async move {
                ...
                headers.insert(RANGE, HeaderValue::from_str(&format!("bytes={}-{}", start, end)).unwrap());
            ...
            match status {
                DownloadStatus::Downloading => {
                    ...
                    if let Err(e) = fs::rename(&temp_path_clone, task_clone.full_path()) {
                        ...
                        let _ = event_sender.send(DownloadEvent::Failed(
                            task_id_clone.clone(),
                            format!("重命名文件失败: {}", e),
                        )).await;
                    } else {
                        ...
                        let _ = event_sender.send(DownloadEvent::Completed(task_id_clone.clone())).await;
                    }
                },
        ...
        // 添加到活跃任务列表
        {
            let mut active_tasks = self.active_tasks.lock().map_err(|_| DownloadError::LockError)?;
    ...
    pub async fn pause_task(&self, task_id: &str) -> Result<(), DownloadError> {
        ...
        // 发送暂停事件
        let _ = self.event_sender.send(DownloadEvent::Paused(task_id.to_string())).await;
    ...
    pub async fn resume_task(&self, task_id: &str) -> Result<(), DownloadError> {
        ...
        if current_status != DownloadStatus::Paused {
            return Err(DownloadError::Other(format!("任务状态不是已暂停: {:?}", current_status)));
        }
        
        // 发送恢复事件
        let _ = self.event_sender.send(DownloadEvent::Resumed(task_id.to_string())).await;
    ...
    pub async fn cancel_task(&self, task_id: &str) -> Result<(), DownloadError> {
        ...
        // 发送取消事件
        let _ = self.event_sender.send(DownloadEvent::Cancelled(task_id.to_string())).await;
    ...
    pub fn get_task_progress(&self, task_id: &str) -> Result<DownloadProgress, DownloadError> {
        ...
        let task = tasks.get(task_id).ok_or_else(|| DownloadError::TaskNotFound(task_id.to_string()))?;
        task.get_progress()
    }
    ...
    async fn get_file_size(&self, url: &str) -> Result<u64, DownloadError> {
        ...
        if !response.status().is_success() {
            return Err(DownloadError::HttpError(format!("HTTP错误: {}", response.status())));
        }
